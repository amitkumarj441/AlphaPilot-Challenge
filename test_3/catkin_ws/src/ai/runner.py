...
from src.utils.data_formater import DataFormater
...
class ReinforcementLearning(object):
    ...
    def __initVars(self, model):
        ...
        self.nb_features = 32  # can be setted via model input shape
    ...
    def loadNominalGatesRef(self):
        with open(os.path.dirname(os.path.realpath(__file__)) +"/src/resources/nominal_gate_locations.yaml") as f:
            return yaml.load(f)
...
model_name = os.path.dirname(os.path.realpath(__file__)) +'/src/graph/to_zip/model.030.h5'
rospy.init_node("alpha", anonymous=True)

ReinforcementLearning(model=model_name).run()
